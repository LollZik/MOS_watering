...
void
spi_ink_write_data(uint8_t cmd, uint8_t *data, uint16_t d_len)
{
...
bool
hal_display_is_busy(void)
{
  return gpio_get(BUSY_PIN) == 1;
}

void
hal_display_write_framebuffer(const uint8_t *framebuf)
{
...
void
hal_display_trigger_refresh(void)
{
  buf[0] = 0xF7; // Full refresh
  spi_ink_write_data(CMD_DISPLAY_UPDATE_CONTROL, buf, 1);
  spi_ink_write_data(CMD_MASTER_ACTIVATION, NULL, 0);
